...
int main() {
    ...
    // Чтение строк из stdin блоками фиксированной длины, пока не достигнут конец файла (EOF)
    while (fgets(chunk, sizeof(chunk), stdin) != NULL) {
        size_t chunk_len = strlen(chunk);

        // Перевыделяем память для буфера
        ...
        // Копируем считанный блок в общий буфер
        memcpy(buffer + buffer_size, chunk, chunk_len);
        buffer_size += chunk_len;
        buffer[buffer_size] = '\0';

        // Проверяем, есть ли символ новой строки в текущем блоке
        if (strchr(chunk, '\n') != NULL) {
            // Обработка каждого символа в строке по мере чтения
            ...
            for (size_t i = 0; i < len; i++) {
                ...
                if (c == ' ' || c == '\t' || c == '\n') { // Если символ - пробел, табуляция или новая строка
                    ...
                    if ((c == '|' && buffer[i + 1] == '|') || (c == '&' && buffer[i + 1] == '&') || (c == '>' && buffer[i + 1] == '>')) {
                        addsym(buffer[++i]); // Добавляем второй символ и продвигаем индекс
                    }
    ...
    free(buffer); // Освобождаем память, выделенную для ввода
}
